const getProductId = decodeURIComponent(location.search);
const getSearchQuery = getProductId.split("=")[1];

const go2MainView = () => {
  return location.replace("./index.html");
};

document.querySelector("#navBtn").addEventListener("click", go2MainView);
...
const displayProducts = (data) => {
  ...
  const groupedCategory = data.reduce((store, value) => {
    ...
    groupedCategory[category].length > 0 &&
      groupedCategory[category].forEach(
        ({
          ...
          cardBtn.addEventListener("click", () => displaySingleProduct(id));
...
const displayProductCategoryLists = (category) => {
  const ulElem = document.querySelector("#lists");

  category.length > 0 &&
    category.forEach(({ title, id }) => {
  ...
  fetch(`https://dummyjson.com/products/category/${category}`, {
    ...
    .then((data) => {
      const categoryProps =
        ...
        data?.products
          .filter(({ id }) => id !== +getSearchQuery)
          .map(({ title, id }) => {
            return { title, id };
          });

      return displayProductCategoryLists(categoryProps);
    })
    .catch((err) => console.error(err.message));
};

const getProductsWithDSpecificId = () => {
  fetch(`https://dummyjson.com/products/${getSearchQuery}`, {
    headers: {
      "Content-Type": "application/json",
    ...
    .then(async (data) => {
      ...
      getProductCategories(data?.category);
    });
};

window.addEventListener("DOMContentLoaded", getProductsWithDSpecificId);
